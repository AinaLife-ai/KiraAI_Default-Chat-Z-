# KiraAI_Default-Chat-Z- 默认消息处理插件优化版 v1.8.2

[![Ask DeepWiki](https://deepwiki.com/badge.svg)](https://deepwiki.com/znq19/KiraAI_Default-Chat-Z-)

修改原版默认所有语音、图片、合并转发都识别的逻辑，减轻小水管模型负担。v1.8.2，KiraAI 2.29.6+ 可用（原生多模态兼容需 2.31.0+）。
...
- **评分加减关键词**（存在感节流区新增 4 项配置）：自定义「加分关键词 / 减分关键词」，命中的用户消息直接给累计分加减分，从而影响评分补正的触发。**只统计用户消息，bot 自己的发言一律不计分。**
  - 词与分值都是列表，一一对应。配对规则：只填一个分值 → 所有词都用它；分值少于词数 → **最后一个分值沿用给后面的词**；分值多于词数 → **多余的忽略**；分值留空 → 默认 5。
